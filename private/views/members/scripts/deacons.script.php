<script>
  ...
  function getAllDeacons(){
    var dataRecord = '';
    var txtSearchInformation = $("#txtsearchdeaconsinfo").val();
    ...
    $.ajax({
      ...
      data: {
        // joinTable: 'churches',
        memberLvlTitle: 'Deacon',
        txtSearchInformation: txtSearchInformation
      },
      beforeSend: function(){
        $("#tbldeaconslist").html("<tr><td class='text-center' colspan='8'>Loading Records...</td></tr>");
      },
      success: function(data){
        ...
        $("#tbldeaconslist").html(dataRecord);
      },
      complete: function(){
  ...
  function selectMember(index){
    Swal.fire({
      title: "Are you sure?",
      text: "Add "+members[index].firstname+" "+members[index].lastname+" in the Board of Deacons?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          type: 'POST',
          url: '<?=ROOT_PUBLIC?>/deaconlist/saveNewDeacon',
          data: {
            defaultid: defaultid,
            memberid: memberid
          },
